"""Train hierarchical model, save checkpoint, then run inference on custom wav files.
...
def compute_loss(text_logits, audio_logits, text_targets, audio_targets,
                 attention_mask, loss_mask=None):
    ...
    for c in range(num_cb):
        cl = F.cross_entropy(
            ...
            audio_targets[:, c].reshape(-1), reduction="none",
        ).view(B, -1)
        cb_losses.append((cl * mask.float()).sum() / denom)
    audio_loss = torch.stack(cb_losses).mean()
    return {"loss": audio_loss, "audio_loss": audio_loss.detach(),
            "cb_losses": [l.detach().item() for l in cb_losses]}


@torch.no_grad()
def generate_from_source(model, mimi, source_codes, num_codebooks, device,
    ...
    text_ids = torch.full((1, T_src), TinyAyaBackbone.ZERO_PADDING,
...
def main():
    ...
    parser.add_argument("--infer_wavs", nargs="*", default=[],
                        help="Wav files to translate after training")
    ...
    loader = torch.utils.data.DataLoader(
        ...
        collate_fn=InterleavedCollator(), num_workers=0, pin_memory=True)
    ...
    while step < args.max_steps:
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            ...
            losses = compute_loss(output["text_logits"], output["audio_logits"],
                                  text_ids, all_codes[:, :args.num_codebooks, :],
                                  mask, loss_mask)
        ...
        if step % args.log_every == 0:
            cb_str = " ".join(f"{l:.2f}" for l in losses["cb_losses"])
            print(f"step {step:5d} | audio {running_loss/args.log_every:.4f} | "
    ...
    if args.infer_wavs:
        ...
        for wav_path in args.infer_wavs:
            ...
            gen_wav, gen_codes = generate_from_source(
                model, mimi, source_codes, args.num_codebooks, device,
