"""Final eval for Stage 2: ASR-BLEU + DNSMOS + demos.
...
def ar_generate(model, src_codes, device, num_codebooks, max_new_frames: int):
    ...
    for _ in range(max_new_frames):
        ...
        text_ids = torch.cat([text_ids,
                              torch.full((1, 1), TinyAyaBackbone.ZERO_PADDING,
...
def load_asr(lang: str, device: str):
    ...
    model_id = {"tr": "openai/whisper-large-v3",
                "hi": "vasista22/whisper-hindi-large-v2"}[lang]
...
def asr_transcribe(proc, mdl, wav_np: np.ndarray, sr: int, device: str) -> str:
    if sr != 16000:
        wav_t = torchaudio.functional.resample(
            torch.from_numpy(wav_np).float(), sr, 16000).numpy()
    else:
        wav_t = wav_np
...
def main():
    ...
    demos_dir = out_dir / "demos"; demos_dir.mkdir(exist_ok=True)
    ...
    # ---- dataset
    ds = StreamingTranslationDataset(args.val_split, model.backbone.tokenizer,
    ...
    for i, row in enumerate(tqdm(rows, desc="eval")):
        ...
        per_sample.append({
            ...
            "direction": direction,
            "ref": ref, "hyp": hyp,
            "ref_norm": normalize_text(ref), "hyp_norm": normalize_text(hyp),
        })
        ...
        if demos_per_dir.get(direction, 0) < args.num_demos:
            ...
            (dd / "4_text.json").write_text(json.dumps({
                "src_text": d.get("src_text"), "tgt_text": ref, "hyp_text": hyp,
    ...
    def _stats(arr):
        ...
        boot = np.stack([a[rng.integers(0, len(a), len(a))].mean(axis=0)
                         for _ in range(200)])
        lo, hi = np.quantile(boot, [0.025, 0.975], axis=0)
        return {"sig": [float(means[0]), float(lo[0]), float(hi[0])],
