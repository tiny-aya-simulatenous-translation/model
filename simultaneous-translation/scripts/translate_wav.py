"""Translate a wav file: encode with Mimi, run model, decode output.
...
def main():
    ...
    output_audio, source_codes = translate(
        backbone, encoder, args.input,
        max_target_frames=args.max_frames, device=device,
    )
    ...
    sf.write(args.output.replace(".wav", "_source_reencoded.wav"), source_reconstructed.numpy(), 24000)
