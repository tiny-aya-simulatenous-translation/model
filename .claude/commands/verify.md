---
description: Run every command in .claude/VERIFY.md and produce a structured pass/fail report.
---

Delegate to the `verifier` subagent (see
`.claude/agents/verifier.md`).

Steps:

1. Confirm `.claude/VERIFY.md` exists and is non-empty.
2. Invoke the `verifier` droid via the Agent tool. It will:
...
5. If all pass, also tick matching items in `.claude/PLAN.md` via
